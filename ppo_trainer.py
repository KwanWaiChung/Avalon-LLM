...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if is_deepspeed_used:
            ...
            if not self.is_peft_model and not (
                ...
                or getattr(
                    self.ref_model.pretrained_model, "is_loaded_in_4bit", False
                )
            ):
    ...
    def _generate_batched(
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            for generation, mask in zip(
                generations, padded_inputs["attention_mask"]
            ):
                if not self.is_encoder_decoder:
    ...
    def step(
        ...
        if self.config.use_score_scaling:
            ...
            score_scaling_factor = (
                self.running.std.to(**tensor_to_kwargs)
        ...
        pbar = tqdm(
            total=self.config.ppo_epochs
            * -(-bs // self.config.backward_batch_size)
            * -(
                -self.config.backward_batch_size // self.config.mini_batch_size
            ),
            disable=self.accelerator.local_process_index != 0,
            desc="Training",
        )
        for _ in range(self.config.ppo_epochs):
            ...
            for backward_batch_start in range(
                ...
                backward_batch_inds = b_inds[
                    backward_batch_start:backward_batch_end
                ]
                ...
                for mini_batch_start in range(
                    ...
                    mini_batch_end = (
                        mini_batch_start + self.config.mini_batch_size
                    )
                    ...
                    mini_batch_dict = {
                        ...
                        "queries": [
                            batch_dict["queries"][i] for i in mini_batch_inds
                        ],
                        "responses": [
    ...
    def gather_stats(self, stats):
        ...
        for k, v in stats.items():
            if isinstance(v, torch.Tensor):
                dist.all_reduce(
                    v.to(self.accelerator.device), dist.ReduceOp.SUM
                )
    ...
    def prepare_model_inputs(
        ...
        if self.is_encoder_decoder:
            ...
            input_data["decoder_attention_mask"] = decoder_inputs[
                "attention_mask"
            ]
        else:
    ...
    def batched_forward_pass(
        ...
        pbar = range(math.ceil(bs / fbs))
        if use_tqdm:
            pbar = tqdm(
                range(math.ceil(bs / fbs)),
                disable=self.accelerator.local_process_index != 0,
                desc="Batched Forward",
            )
        for i in pbar:
            input_kwargs = {
    ...
    def compute_advantages(
        ...
        for t in reversed(range(gen_len)):
            ...
            delta = (
                rewards[:, t] + self.config.gamma * nextvalues - values[:, t]
            )
            lastgaelam = (
    ...
    def loss(
        ...
        return_mean, return_var = masked_mean(returns, mask), masked_var(
            returns, mask
        )
        value_mean, value_var = masked_mean(values, mask), masked_var(
            values, mask
        )
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        response_lens = torch.tensor(
            [len(response) for response in data["responses"]],
            dtype=torch.float,
        )

        stats["tokens/queries_len_mean"] = (
            torch.mean(query_lens).cpu().numpy().item()
        )
        stats["tokens/queries_len_std"] = (
            torch.std(query_lens).cpu().numpy().item()
        )
    ...
    def log_stats(
        ...
        if self.config.log_with == "wandb":
            ...
            batch_list = [
                batch[column_to_log] for column_to_log in columns_to_log
            ]
            if self.is_distributed:
                gathered_batch_list = []
                for b in batch_list:
        ...
        if self.accelerator.is_main_process:
            ...
            if "query" not in batch.keys() and "response" not in batch.keys():
                ...
                table_rows = [
                    list(r) for r in zip(*batch_list, rewards.cpu().tolist())
                ]
                logs.update(
                    {
                        "game_log": wandb.Table(
                            columns=[*columns_to_log, "reward"],
                            rows=table_rows,
                        )
                    }
                )
    ...
    def _show_tokens(self, tokens, masks):
        ...
        for _i, (token, mask) in enumerate(zip(tokens, masks)):
            if mask == 1:
                ...
                text.append(
                    self.tokenizer.decode(token.item()), style="black on cyan3"
                )
                text.append(" ")
        print(text)
