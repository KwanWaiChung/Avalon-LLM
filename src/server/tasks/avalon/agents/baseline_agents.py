...
class NaiveMinion(NaiveAgent):
    ...
    async def vote_on_mission(
        self, mission_id: int, team: frozenset, **kwargs
    ):
...
class NaiveServant(NaiveAgent):
    ...
    def __init__(
        ...
        **configs
        ...
        self.player_side_probabilities = [
            1 / len(self.possible_player_sides)
        ] * len(self.possible_player_sides)
    ...
    def generate_possible_player_sides(self, sides: List[int], num_evils: int):
        ...
        if -1 not in sides:
            ...
            for side in [0, 1]:
                ...
                if side == 0:
                    out.extend(
                        self.generate_possible_player_sides(
                            sides_copy, num_evils - 1
                        )
                    )
                else:
                    out.extend(
                        self.generate_possible_player_sides(
                            sides_copy, num_evils
                        )
                    )
            return out
    ...
    async def vote_on_team(self, team: frozenset, mission_id: int, **kwargs):
        ...
        return (
            1
            if team in self.find_most_prefered_teams(self.team_preferences)
            else 0
        )
    ...
    async def propose_team(self, mission_id: int, **kwargs):
        ...
        return random.choice(
            self.find_most_prefered_teams(self.team_preferences)
        )
