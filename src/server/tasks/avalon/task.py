...
class AvalonBench(Task):
    def __init__(
        self, num_players, agent_list, discussion, data_file, **configs
    ):
    ...
    async def start_sample(
        ...
        sessions = [
            AvalonSessionWrapper(session, proxy)
        ...
        try:
            while not env.done:
                ...
                print(
                    ColorMessage.orange(
                        f"##### Mission {env.turn}, Round {env.round} #####"
                    )
                )
                ...
                # if phase is team voting phase, ask for votes
                    ...
                    game_env_log.append(
                        f"Team votes at this round: {str(votes)}"
                    )
                ...
                # if phase is quest voting phase, ask for votes
                    ...
                    game_env_log.append(
                        f"Quest votes at this round: {str(votes)}"
                    )
            ...
            for idx, player in enumerate(player_list):
                proxy.set_current_agent(idx)
                if idx == llm_idx:
                    llm_believed_player_sides = (
                        await player.get_believed_sides(
                            num_players=self.num_players
                        )
                    )
